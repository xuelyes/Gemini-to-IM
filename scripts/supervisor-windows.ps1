<#
.SYNOPSIS
  Windows daemon manager for gemini-to-im bridge.
...
# ── Paths ──
$CtiHome    = if ($env:CTI_HOME) { $env:CTI_HOME } else { Join-Path $env:USERPROFILE '.gemini-to-im' }
...
$ErrLogFile = Join-Path (Join-Path $CtiHome 'logs') 'bridge-error.log'
$DaemonMjs  = Join-Path (Join-Path $SkillDir 'dist') 'daemon.mjs'

$ServiceName = 'GeminiToIMBridge'
